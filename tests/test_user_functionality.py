...
def test_user_register_succeed(client):
...
def test_wrong_user_payload(client):
    new_user = {
        "username": "user4",
    }
...
def test_get_user_succeeded(client):
    user_id = 1
...
def test_get_user_unsucceeded(client):
    user_id = 10
...
def test_delete_user_404(client):
    ...
    response1 = client.post(
        endpoint1, json={"username": "maryam", "password": "pass4rd"}
    )

    token = response1.json.get("refresh_token")
    user_id = 10
...
def test_delete_user_succeeded(client):
    # Providing access token
    endpoint1 = "/login"
    response1 = client.post(
        endpoint1, json={"username": "maryam", "password": "pass4rd"}
    ...
    delete_response = client.delete(
        endpoint2, headers={"Authorization": f"Bearer {token}"}
    )
